...
from backend.old_pipeline.feature_engineer import load_and_engineer_data
...
class PatchTSTModel(nn.Module):
    def __init__(
        ...
        self.company_embedding = nn.Embedding(
            num_embeddings=num_companies,
            embedding_dim=COMPANY_EMBED_DIM
        )
...
class TFTModel(nn.Module):
    ...
    def forward(self, x, company_id):
        ...
        context = torch.cat([context, company_emb], dim=1)

        return self.head(context)


def get_latest_company_csv(company):
...
def prepare_all_company_data(companies):
    all_frames = []

    for company in companies:
        csv_path = get_latest_company_csv(company)

        if csv_path is None:
            print(f"[SKIPPED] No CSV found for {company}")
            continue

        print(f"[PROCESSING] {company}: {csv_path}")
        ...
        try:
            ...
            df["future_close_week"] = df["Close"].shift(-WEEK_STEPS)
            df["future_close_month"] = df["Close"].shift(-MONTH_STEPS)

            df = df.dropna().reset_index(drop=True)

            if len(df) < SEQ_LEN + MONTH_STEPS:
                print(f"[SKIPPED] Not enough rows for {company}")
                continue

            processed_path = os.path.join(
                PROCESSED_DIR,
                f"{company}_featured.csv"
            )
            ...
            print(f"[ERROR] Failed processing {company}: {e}")

    if not all_frames:
        raise ValueError("No company data available for training.")

    combined = pd.concat(all_frames, ignore_index=True)

    combined_path = os.path.join(
        PROCESSED_DIR,
        "all_companies_featured.csv"
    )
...
def prepare_ml_arrays(df, feature_cols, scaler=None, fit_scaler=False, company_map=None):
    ...
    if company_map is None:
        ...
        company_map = {
            company: idx
            for idx, company in enumerate(company_codes)
        }
...
def evaluate_model(model, loader):
    ...
    week_actual_direction = np.sign(week_actuals - last_closes)
    week_pred_direction = np.sign(week_preds - last_closes)

    month_actual_direction = np.sign(month_actuals - last_closes)
    month_pred_direction = np.sign(month_preds - last_closes)

    week_direction_accuracy = (
        week_actual_direction == week_pred_direction
    ).mean() * 100

    month_direction_accuracy = (
        month_actual_direction == month_pred_direction
    ).mean() * 100
...
def train_global_and_select_best():
    print("=" * 70)
    print("PHASE 2 GLOBAL MODEL TRAINING STARTED")
    print("=" * 70)

    combined_df, combined_path = prepare_all_company_data(COMPANIES)
    ...
    for model_name, model in models.items():
        ...
        record = {
            ...
            "train_week_mae": train_metrics["week_mae"],
            "train_week_rmse": train_metrics["week_rmse"],
            "train_week_direction_accuracy": train_metrics["week_direction_accuracy"],

            "train_month_mae": train_metrics["month_mae"],
            "train_month_rmse": train_metrics["month_rmse"],
            "train_month_direction_accuracy": train_metrics["month_direction_accuracy"],
    ...
    best_row_idx = metrics_df["test_combined_score"].idxmax()
    best_model_name = metrics_df.loc[best_row_idx, "model"]

    metrics_df.loc[best_row_idx, "selected"] = "Yes"

    best_model = trained_models[best_model_name]

    print("\n" + "=" * 70)
    print(f"BEST GLOBAL MODEL SELECTED: {best_model_name}")
    print("=" * 70)
    ...
    best_info = {
        ...
        "improvements": [
            ...
            "gradient_clipping",
            "company_embeddings",
            "shorter_month_horizon"
        ],
        "selection_metric": "test_combined_score",
        "selected_model_metrics": metrics_df.loc[best_row_idx].to_dict()
    }
...
if __name__ == "__main__":
    ...
    result = train_global_and_select_best()
